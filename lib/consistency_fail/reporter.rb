...
#    end
    ...
    def header(table_header_text, longest_model_length)
      puts
      divider(longest_model_length * 2)
      puts "validates_uniqueness calls not backed by unique indexes"
      divider(longest_model_length * 2)
      print table_header_text.ljust(longest_model_length + 2)
    ...
    def report_validates_uniqueness_problems(indexes_by_model)
      if indexes_by_model.empty?
        ...
        longest_model_length = indexes_by_table_name.map(&:first).
                                                     ...
                                                     last.
                                                     length
        longest_model_length = [longest_model_length, table_header_text.length].max

        header(table_header_text, longest_model_length)
